...
**ルール1【最高優先度】委員長・議長の指名文**
`〇〇君。` `〇〇委員。` `〇〇さん。` `〇〇大臣。` `〇〇副大臣。` `〇〇政務官。` だけの短い1文、役職付き指名文（例:「財務大臣片山さつき君。」）、「続いて〇〇君。」等 → 必ず**委員長**のsplitを作成。指名文の直後は指名された人物の別splitを作成。主発言者の名前でも委員長splitにする。

**ルール2: Whisper話者ラベル行**
- `〇〇委員長：text` 等コロン付き行 → そのラベル行番号からその話者の発言開始
- `〇〇（役職名）` のみの行 → そのラベル行番号からその話者の発言開始
- line 0が非ラベルで line 1がWhisperラベルの場合、line 0はそのラベル話者に帰属（特に委員長・議長の議事進行文）

**ルール3: 答弁冒頭の定型句**
「お答えいたします」「お答え申し上げます」「御指摘の点について」「ただいまの御質問に」等 → 答弁者または政府参考人。

**ルール4: 発言の継続**
ルール1〜3に該当しない文は直前の話者の継続。発言内に他者の名前が登場しても話者交代ではない。
...
| 質疑者 | 委員・議員の質問・意見 |
| 答弁者 | 大臣・副大臣・政務官・閣僚の答弁 |
| 政府参考人 | 省庁の局長・部長・審議官等の答弁 |
| 参考人 | 独立機関の官僚・外部有識者・民間人 |
| その他 | 上記以外 |

- 大臣・副大臣・政務官・官房長官 → 必ず`答弁者`（`質疑者`は使わない）
- 省庁の局長・部長等 → `政府参考人`
- 裁判所・会計検査院等 → `参考人`

## 過剰分割の防止
- splitはルール1〜3に明確に該当する場合のみ作成
- 同じ話者の発言は1つのsplitに統合（テーマが複数でも分割しない）
- 通常2〜6 splits/セグメント。10件超は過剰分割を疑い統合すること

## 出力形式
```json
{"splits": [{"start": 0, "speaker": "発言者名", "role": "役割"}, ...]}
```
- start: 0始まりの文番号。splitsはstart昇順。最初のsplitのstartは必ず0。
"""
