...
要件:
- **冒頭の一文**に院名・委員会名（「## セッション情報」の値をそのまま使う）を必ず明記する（例:「衆議院○○委員会において、...」。参考人質疑・所信表明等は種別を添える）
...
**抽出しない**: 努力表明（〜に取り組む/推進する/努める）、検討・協議の表明、意向・希望形（〜したい/まいりたい）、現状報告（〜しているところ）、議事進行、一般方針

**主節述語テスト**: 文末の主節述語が意向形（〜まいりたい/したい）なら、文中に具体的数値・日時があっても抽出しない。
...
- 参考人・外部有識者は除外
- speakerは入力「回答者:」の実名をそのまま使用（汎用名禁止）
- qa_idは入力のIDをそのまま使用
