...
以下のJSON形式で出力してください（speaker, party, roleは返さないこと。抽出できるペアがない場合は `{"pairs": []}` を返すこと）:
{
  "pairs": [
    {
      "topic": "質疑テーマ（簡潔に）",
...
**intent（必須・全ペアに必ず含めること）**の分類基準:
- fact_check: 大臣等の過去発言との齟齬・数値の正確性・事実認識の一致を問う質問
- policy_proposal: 新しい政策の導入・制度変更・予算措置・法改正等を提案または実施を求める質問
- accountability: 過去の政策判断・行政の不作為・公約違反・矛盾発言等について責任や説明を求める質問
- information_request: 現状の数字・制度内容・政府見解・計画・資料や文書の開示等を求める質問
- other: 上記いずれにも当てはまらないもの

重要なルール:
- 質疑者が複数の**異なるテーマ**（例: 道路整備と航空政策）について質問した場合、テーマごとに別のQ&Aペアを作成すること
- 1つも漏らさずに抽出すること（ただしQ&A構造として成立しないものは除く）
- 答弁が空・極端に短い・単なる相槌のみのQ&Aペアは含めないこと
- **Q&Aペアの質問者と答弁者は別々の人物でなければならない**。同一人物の発言のみからペアを構成しないこと
- **趣旨説明・所信表明・法案説明等の一方的な演説はQ&Aペアとして抽出しないこと**。問いかけと応答の往復が必要
- full_textは返さないこと。代わりにsentence_indices（文番号の配列）を返すこと
- sentence_indicesは、入力の(N)の番号を配列で指定。そのQ&Aの該当部分の文だけを選ぶこと
  - **挨拶・自己紹介・感謝**（「〇〇です」「ありがとうございます」「早速質問に入りたい」等）はsentence_indicesに**含めないこと**
  - **質問の文脈・背景説明**（現状説明・問題提起・制度の説明）は、答弁者が文脈を正しく理解するために必要な文として**含めること**
- 1つのutteranceに複数テーマが含まれる場合（例: 代表質問）、テーマごとに該当する文だけを選択すること
- summaryの箇条書きは**質疑者が実際に問いかけている内容のみ**を記載すること（挨拶・背景説明・フレーミングは含めない）
...
要件:
- **冒頭の一文**に必ずセッション種別・院名・委員会名を明記すること。
  - 院名・委員会名は入力の「## セッション情報」に記載された値をそのまま使うこと
    （例: 院=衆議院, 委員会=本会議 → 「衆議院本会議において、...」）
    （例: 院=衆議院, 委員会=法務委員会 → 「衆議院法務委員会において、...」）
  - セッション種別の判定基準（Q&Aペアの内容から判断）:
    - 全回答者のroleに「参考人」が含まれる → 「参考人質疑」（例:「衆議院○○委員会の参考人質疑において、...」）
    - Q&Aペアがない、またはセッション概要に「所信表明」と記載 → 「所信表明」（例:「衆議院○○委員会において（所信表明）、...」）
    - 委員会名が「憲法審査会」 → 「憲法審査会」（「各党間の多党間討議であり、政府への質疑応答ではない」旨を必ず明記すること）
    - 委員会名が「本会議」 → 「本会議」として冒頭に明記すること（「衆議院本会議において、...」）
    - それ以外 → 種別ラベルは不要。「衆議院○○委員会において、...」と委員会名のみ冒頭に書けばよい
- 主要な答弁者（大臣名・参考人名等）と主要テーマを含める
- **複数テーマがある場合は必ず全テーマに言及すること**（1つのテーマだけを詳述して他を省略してはいけない）
- 入力に含まれる全Q&Aペアのトピックを確認し、未言及のトピックがないかチェックしてから出力すること
- 後期高齢者・高額療養費・金融所得反映・妊婦健診・医療物資など複数の独立した制度・施策が含まれる場合、それぞれを1文以上で言及すること
- 個別の質問詳細ではなくセッション全体のフレーミングを書く
- 3-5文、装飾なしの本文のみ
- 出力前に「入力の各トピックをカバーしたか？」を自己確認すること
"""
...
- トピック数はQ&Aペア数に応じて動的に決める:
  - Q&Aペア数が **5件以下** の場合: 政策的に異なるテーマは原則として**1テーマ = 1トピック**とする（"OTC薬の保険適用" と "医療物資の供給" のような明確に異なる政策領域は絶対に統合しない）
  - Q&Aペア数が **6〜50件** の場合: 1トピックあたり 3〜10件 を目安に集約する
  - Q&Aペア数が **51件以上** の場合: 1トピックあたり 10〜20件 を目安に集約する
- 2つのQ&Aペアを統合してよいのは、**まったく同一の法案・制度・施策**について質疑している場合のみ。類似する政策領域であっても対象となる制度・物資・対象者が異なれば別トピックとする
- 政策領域・法案・社会問題などの大テーマだけでなく、個別質疑者が専門的に追及したサブテーマも独立したトピックとして設ける
