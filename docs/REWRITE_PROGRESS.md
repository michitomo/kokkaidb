...
| PR45 | metrics=null 軽減: ConcreteItem.type の Pydantic Literal coerce (Session #19 起票) | 🟢 小 | ✅ | michitomo/structurer-rewrite-plan | 2026-05-11 | `_sanitize_metrics_data` 新設。LLM が ConcreteItem.type に "date"/"organization" 等の想定外値を返したとき `_score_one_pair` の初回 `model_validate` 失敗後に sanitize して再試行。F3 audit で 14-24% のペアが null だった問題を軽減 |
| PR46 | answer 内の純粋な話者ラベル行除去 (残課題対応) | 🟢 小 | ✅ | michitomo/structurer-rewrite-plan | 2026-05-11 | `_PURE_LABEL_LINE_RE` + `_strip_pure_label_lines` 新設。複数話者が1 answer に統合されたとき挿入される「\n砂原参考人。\n」等の純粋ラベル行を除去。テスト +5件 |
| PR47 | カタカナ名・1字姓・bare名前：・君。敬称 (過学習チェック audit 起票) | 🟢 小 | ✅ | michitomo/structurer-rewrite-plan | 2026-05-11 | 11 新規セッション overfit check audit で発見。(1) 全パターンに `_JP_NAME_CHARS=[一-鿿ぁ-ゟァ-ヿ]` でカタカナ名対応、(2) 1字姓 [漢字/カタカナ]{1,20}? で「林大臣。」捕捉、(3) bare `名前：` Pattern D、(4) `名前君。` 委員会非公式敬称。**過学習チェック結論: 誤 strip ゼロ件、過学習の証拠なし**。テスト +9件 |
