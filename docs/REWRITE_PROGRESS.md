...
| **#17 (追加修正 4)** | PR42 bug fix + PR43 実装 | PR42 fix / PR43 | 🔄 実施中 (2026-05-11): regex `{0,12}→{0,20}` + 全角コロン追加 + speaker_name prefix strip。PR43 trailing label strip 新設。F1 → F2 regen 予定 |
| **#18 (F2 再走)** | 6 件で再評価 | (実装なし) | F2 ゲート (avg ≤ 8) 通過 |
